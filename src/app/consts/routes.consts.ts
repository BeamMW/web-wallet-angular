export enum routes {
    FTF_PASSWORD_CREATE_ROUTE = '/initialize/create-password',
    FTF_CONFIRM_SEED_ROUTE = '/initialize/confirm-seed',
    FTF_GENERATE_SEED_ROUTE = '/initialize/generate-seed',
    FTF_VIEW_SEED_ROUTE = '/initialize/view-seed',
    FTF_CREATE_WALLET_ROUTE = '/initialize/create',
    // RESTORE_WALLET_ROUTE = '/initialize/restore',

    WALLET_MAIN_ROUTE = '/wallet/main',

    TRANSACTION_DETAILS_ROUTE = '/transactions/details',
    TRANSACTIONS_LIST_ROUTE = '/transactions/view',

    SEND_AMOUNT_ROUTE = '/send/amount',
    SEND_ADDRESSES_ROUTE = '/send/addresses',
    SEND_CONFIRMATION_ROUTE = '/send/confirmation',

    SETTINGS_ALL_ROUTE = '/settings/all',
}
